...
def new_search(request):
    ...
    item_price = soup.find(class_ = 'elements-offer-price-normal__price').get_text()
    ...
    for items in items_list:
        ...
        item_price = items.find(class_ = 'elements-offer-price-normal__price').get_text()
        item_url = items.find(class_ = 'elements-title-normal one-line').get('href')
        # item_review = items.find(class_ = 'seb-supplier-review text-ellipsis list-no-v2-decisionsup__element').get_text()

        final_item_list.append((item_name, item_price, item_url, item_review))
